...
def findshows(direct,s = ''):
    ...
    newnames = [compressname(re.search(regex,show).group(1)) for show in shortpaths]
    newseasons = [compressseason(re.search(rseason,show).group(1)) for show in shortpaths]
    endings = [re.search(rending,show).group(1) for show in shortpaths]
